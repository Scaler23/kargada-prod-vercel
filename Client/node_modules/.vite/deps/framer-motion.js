...
var BaseAnimation = class {
   ...
   */
  onKeyframesResolved(keyframes2) {
    const { name, type, velocity, delay: delay2, onComplete, onUpdate } = this.options;
    if (!canAnimate(keyframes2, name, type, velocity)) {
      if (instantAnimationState.current || !delay2) {
        const finalKeyframe = getFinalKeyframe(keyframes2, this.options);
...
var MainThreadAnimation = class extends BaseAnimation {
  ...
  tick(timestamp, sample = false) {
    const { generator, mirroredGenerator, mapPercentToKeyframes, keyframes: keyframes2, calculatedDuration, totalDuration, resolvedDuration } = this.resolved;
...
var AcceleratedAnimation = class extends BaseAnimation {
  constructor(options) {
    super(options);
    const { name, motionValue: motionValue2, keyframes: keyframes2 } = this.options;
    this.resolver = new DOMKeyframesResolver(keyframes2, (resolvedKeyframes) => this.onKeyframesResolved(resolvedKeyframes), name, motionValue2);
    this.resolver.scheduleResolve();
  }
  initPlayback(keyframes2) {
    ...
    if (this.pendingTimeline) {
      ...
      animation.onfinish = () => {
        const { onComplete } = this.options;
        motionValue2.set(getFinalKeyframe(keyframes2, this.options));
        onComplete && onComplete();
        this.cancel();
        this.resolveFinishedPromise();
        this.updateFinishedPromise();
